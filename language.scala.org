...
高階型を定義する場合に有効化します。
<<<<<<< HEAD
強力な型の表現が可能になりますが、型推論も難しくなるし、やりすぎるとコードも読みにくくなります。
初学者には難しいので避けましょうとのことです。
=======
強力な型の表現が可能になりますが、Scalaでは型推論も難しいため、やりすぎるとコードも読みにくくなります。
初学者には難しいので、慎重な利用を促すために明示的な有効かが必要となりました。
>>>>>>> 8516800... remove symbolic link
...
<<<<<<< HEAD
強力すぎる機能については乱用を避け、注意深く責任を持って使用しましょうという、Odersky先生からのメッセージですね。

ScalaはライブラリやDSLを開発する側とそれらを利用する側のそれぞれの視点を考慮して作られています。
=======
